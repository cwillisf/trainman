...
#	4) Adding Finite State Machine logic to determine stove burn state
#	5) Adding automation of Air Control Lever using stepper motor
#
# Hardware:
...
owhost      = "192.168.62.17"			# IP address of the OWServer
owoid       = "iso.3.6.1.4.1.31440.10.5.1.1.1"	# Use snmp walk to figure this out
owcommunity = "panda"				# Default is typically public

# State Variables: Determine state of stove
pipemin  = 225		# Minimum pipe temp for good burn, if < set air 100% (COAL/WARM)
pipemax  = 500		# Maximum pipe temp for safe burn, if > set air 0%   (PMAX)
stovemin = 400		# Minimum stove temp for efficient burn, if < reg to pipetemp (PIPE)
stoveset = 500		# Target temperature for stove, its like a stove thermostat setting.
stovemax = 700		# Maximum stove temp for safe burn, if > decrease air (SMAX)
coldtemp = 100		# Pipe < coldtemp is COOL, Both stove and pipe < coldtemp is COLD, set air 0%
air      = 999

# Air Control Variables: Used by aircontrol stepper motor
airsteps = 468		# Number of MICRO steps used to move air control lever
...
def set_display_text():
    ...
    messagelist[3] = "A"+str(air)
    messagelist[4] = "F"+str(fanspeed)+"P"+str(fanpro)
    messagelist[5] = state
...
# Control airlever
def set_air_control(aircontrol):
    global air, airsteps

    aircontrol = max(min(aircontrol,airsteps),0)	# Limit new airsetting 0 <= aircontrol <= airsteps
    steps = aircontrol - air				# Number of steps to change

    if (steps > 0):
        print "Increasing aircontrol by",steps,"from",air,"to",aircontrol
        myStepper.step(steps, Adafruit_MotorHAT.FORWARD, Adafruit_MotorHAT.MICROSTEP)
    elif (steps < 0):
        steps = abs(steps)
        print "Decreasing aircontrol by",steps,"from",air,"to",aircontrol
...
# Determine burn state of stove using stove and pipe temp as well as air control position.
def determine_state():
    global air
    stovepercent = 100 * (stovetemp - coldtemp) / (stovemin - coldtemp)
    pipepercent = 100 * (pipetemp - coldtemp) / (pipemin - coldtemp)
    pipelimit = 100 - (100 * (pipetemp - pipemin) / (pipemax - pipemin))
    pipelimit = min(100,pipelimit)
    pipelimit = max(0,pipelimit)

    state = "UNKN"

    # If both overheated set air=min
    if ((stovetemp >= stovemax) and (pipetemp >= pipemax)):
        state = "MAX"
        air = 0
        return state

    # If stove overheated set air=min
    if (stovetemp >= stovemax):
        state = "SMAX"
        air = 0
        return state

    # If pipe overheated set air=min
    if (pipetemp >= pipemax):
        state = "PMAX"
        air = 0
        return state

    # Following is not quite correct.  pipe should not be relative
    # Progression should be: COLD->WARM->PIPE->STOV->COAL->COOL->COLD 
    # Need it to stay in STOV MODE unless OVER PIPE Temp.
    # BOTH STOVE AND PIPE NEED TO BE DYNAMIC (like flying a plane)
    # Air steps up if both stove and pipe say up
    # Air steps down if either stove or pipe say down
    # Else Air stays same
    #
    # If pipe is above pipemin then temp is reglated dynamically
    if (pipetemp > pipemin):
        # If stove not reached stoveset set air according to pipetemp
        if (stovetemp < stoveset):
            state = "PIPE"
            air = pipelimit
            return state
        # If stove is at stove setpoint set air dynamically to maintain stoveset
        else:
	    state = "STOV"
            air = pipelimit
            return state

    # If stove and pipe are below cold temmp then cold
    if ((stovetemp < coldtemp) and (pipetemp < coldtemp)):
        state = "COLD"
        air = 0
        return state

    # If pipe is below cold temp but not stove then cool
    if (pipetemp < coldtemp):
        state = "COOL"
        air = 0
        return state

    # If pipe high relative to stove then warming up
    if (pipepercent >= stovepercent):
        state = "WARM"
        air = 100
        return state
    else:
    # If pipe low relative to stove then burning coals
        state = "COAL"
        air = 100
        return state
...
while True:
    ...
    pipelimit = min(100,pipelimit)
    pipelimit = max(0,pipelimit)
    pipeair   = pipelimit * airsteps / 100

    # Print variables. + concatination and str() to gets rid of extra spaces after = symbols
    print "stovetemp="+str(stovetemp)+" pipetemp="+str(pipetemp)+" roomtemp="+str(roomtemp)+" air="+str(air)+" fanspeed="+str(fanspeed)+" fanpro="+str(fanpro)+" bright="+str(bright)+" state="+state +" pipelimit="+str(pipelimit)+" pipeair="+str(pipeair)

    # Write text to the LED displays
    set_display_text()

    # Test air
    #set_air_control(air-1)
    if (x % 30) == 0:
        if (air != pipeair):
            set_air_control(pipeair)
